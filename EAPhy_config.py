...
bestcontigPath = os.path.join(homePath, 'specify_the_name_of_your_contigs_folder')
...
individuals = os.path.join(homePath,'specify_the_name_of_your_text_file_with_indivs')

# Specify the list with all loci to be included in the analysis, names should exactly match the Fasta files
exons = os.path.join(homePath, 'specify_the_name_of_your_text_file_with_exons')

# Basename of output folder and all outputfiles 
out_name = "specify_the_name_of_your_run_specific_output_folder"
...
column_gap_window = 7           # Jump-sliding-window length (in amino acids) used to filter out codon columns with missing data, USE UNEVEN NUMBER SINCE ACCEPT/REJECT RATIO DEPENDS ON WHETHER MORE THAN HALF COLUMNS GAPS PRESENT/ABSENT + IF YOU SET 1, NOTE THAT CODONS WITH HETEROZYGOUS SITES LEADING TO A NON-SYNON. SUBSTITUTION DEPENDENT ON ALLELE ARE REMOVED! NEEDS FIXING
column_gap_ratio = 0.1          # Gap ratio (gaps/column) used to filter out codon columns with missing data
