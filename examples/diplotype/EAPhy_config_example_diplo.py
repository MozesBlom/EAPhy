...
bestcontigPath = os.path.join(homePath, 'examples', 'diplotype', 'test_ambig_contigs')
...
individuals = os.path.join(homePath,'examples', 'test_indivs_diplo_list.txt')
...
id_change_list = os.path.join(homePath,'examples', 'test_id_change_diplo.txt')               # Path to a tab-delim text file where every current ID has a corresponding new name (two columns, first column head: old_ID, second column head: new_ID), that is less than 10 characters long
